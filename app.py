...
# Envío de órdenes a SAP con Session y $metadata
...
def submit():
    ...
    try:
        ...
        meta_resp = sl.get(f"{SERVICE_LAYER_URL}/$metadata")
        meta_resp.raise_for_status()
        if '<EntitySet Name="Orders"' not in meta_resp.text:
            raise RuntimeError("El recurso Orders no está disponible en $metadata; revisa su nombre exacto.")
        ...
        app.logger.error(f"Runtime error SL: {rt_err}", exc_info=True)
        flash(str(rt_err), "error")
        return redirect(url_for('dashboard'))
    data     = resp.json()
